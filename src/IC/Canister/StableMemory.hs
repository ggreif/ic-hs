...
type HostM s = IO (Either String s)
...
checkAccess mem offset len = do
  n <- memorySizeInBytes mem
  if ((fromIntegral offset :: Integer) + fromIntegral len > fromIntegral n)
  then return $ Left "stable memory error: out of bounds"
  else return $ Right ()
...
read mem ptr len = do
  ...
  do
    repr <- readIORef mem
    return $ Right $ toByteString repr ptr len
...
write mem ptr blob = do
  ...
  Right <$> modifyIORef' mem (\repr -> repr { smWrites = (ptr, BL.toStrict blob) : smWrites repr })
